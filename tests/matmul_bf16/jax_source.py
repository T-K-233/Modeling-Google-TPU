...
def matmul_kernel(x_ref, y_ref, z_ref):
    # BF16 x BF16 -> FP32 accumulation/output
    z_ref[...] = (x_ref[...].astype(jnp.float32) @ y_ref[...].astype(jnp.float32))


@jax.named_call
def matmul_bf16(x: jax.Array, y: jax.Array):
    m, k = x.shape
    k2, n = y.shape
    assert k == k2
...
b = jnp.arange(128 * 8, dtype=jnp.float32).reshape(128, 8).astype(jnp.bfloat16)
